...
export default async function GoalPage({ params }: { params: { goal: string } }) {
  ...
  return (
    <main className="min-h-screen bg-[#f0faf8]">
      <Header />
      <div className="mx-auto max-w-4xl px-4 pb-16 pt-8 sm:px-6 lg:px-8">
        <Link href="/get-started" className="text-sm font-bold uppercase tracking-wide text-teal-700 hover:text-teal-900">
          {t('back')}
        </Link>
        <div className="mt-8 rounded-3xl border-2 border-teal-100 bg-white p-10 shadow-sm shadow-teal-900/5">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900">{t(`goals.${params.goal}.title`)}</h1>
          <p className="mt-4 text-lg text-slate-600">{t(`goals.${params.goal}.description`)}</p>
          <Link
            href="/learn"
